    ...
    int main(int argc, char **argv)
    {
        ...
        // Obtener los datos de la tabla sensores_pc de la base de datos local
        ...
        while (sqlite3_step(statement) == SQLITE_ROW) {
            ...
            string sensor = reinterpret_cast<const char *>(sqlite3_column_text(statement, 1));
            float minimo = static_cast<float>(sqlite3_column_double(statement, 2));
            float promedio = static_cast<float>(sqlite3_column_double(statement, 3));
            float maximo = static_cast<float>(sqlite3_column_double(statement, 4));
            string fecha = reinterpret_cast<const char *>(sqlite3_column_text(statement, 5));
        ...
        cout << "Base de datos copiada exitosamente a la nube" << endl;
